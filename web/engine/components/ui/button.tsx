...
const buttonVariants = cva(
  "inline-flex items-center justify-center whitespace-nowrap font-bold transition-all duration-300 ease-[cubic-bezier(0.4,0,0.2,1)] disabled:pointer-events-none [&_svg]:pointer-events-none [&_svg]:shrink-0 shrink-0 outline-none focus-visible:ring-2 focus-visible:ring-brand/20",
  {
    variants: {
      variant: {
        // 토스 스타일: brandSolid — 키컬러 배경, 흰 텍스트
        default:
          "bg-brand text-white hover:bg-brand/90 active:bg-brand/80 disabled:bg-surface-muted disabled:text-text-disabled dark:bg-brand/80 dark:text-[#1e1e2e] dark:hover:bg-brand",
        ...
        // outline — 테두리만
        outline:
          "border border-border bg-transparent text-text-primary active:bg-surface-muted/50 disabled:border-surface-muted disabled:text-text-disabled",
        // ghost — 배경/테두리 없음
        ghost:
      ...
      size: {
        ...
        sm: "h-9 px-3.5 gap-1 text-[14px] rounded-full [&_svg:not([class*='size-'])]:size-3.5",
        md: "h-10 px-4 gap-1.5 text-[14px] rounded-full [&_svg:not([class*='size-'])]:size-4",
        lg: "h-[52px] px-5 gap-2 text-[18px] rounded-full [&_svg:not([class*='size-'])]:size-[22px]",
